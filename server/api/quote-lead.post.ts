...
export default defineEventHandler(async (event) => {
  ...
  function buildAttributionRows(attr: Record<string, string> | null | undefined): string {
    ...
    if (attr.landing_page) { const lpPath = attr.landing_page.split('?')[0]; rows.push(`<tr><td style="padding:6px 0;color:#6b7280;font-size:14px;white-space:nowrap;">Landing Page</td><td style="padding:6px 0;font-size:13px;color:#666;word-break:break-all;overflow-wrap:break-word;max-width:220px;">${lpPath}</td></tr>`) }
    return rows.join('')
  }
